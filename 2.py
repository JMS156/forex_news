...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Try to wait for at least one known element to be sure content is loaded
        await page.wait_for_selector("tr.calendar__row")
        ...
        for row in rows:
            try:
                ...
                impact_text = await impact.get_attribute("title") if impact else "None"

                print(f"[{impact_text}] {time_text} - {currency_text} - {title_text}")
            except:
                continue
